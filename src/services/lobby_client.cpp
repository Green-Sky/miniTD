...
bool LobbyClient::enable(MM::Engine& engine, std::vector<MM::UpdateStrategies::TaskInfo>& task_array) {
...
void LobbyClient::disable(MM::Engine& engine) {
	// *shrug*
}

void LobbyClient::update(MM::Engine& engine) {
	auto* net = engine.tryService<MM::Services::NetChanneledInterface>();
...
bool LobbyClient::handleJoin(std::string_view scheme, std::string_view rest_of_uri) {
	assert(!scheme.empty());
...
bool LobbyClient::registerHandler(std::string_view uri_scheme, MM::Engine::service_id_type service_id) {
	_uri_handlers[std::string{uri_scheme}] = service_id;
	return true; // TODO: error checking
}

bool LobbyClient::join(std::string_view uri) {
